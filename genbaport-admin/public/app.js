...
const state={report:{},invoice:{},photos:[]};
...
function renderInvoice(){const d=invoiceData();$('#pv-invoice-no').textContent=d.invoiceNo||'—';$('#pv-issue-date').textContent=d.issueDate?`発行日 ${d.issueDate}`:'';$('#pv-client').textContent=(d.client||'')||'御中';$('#pv-issuer').textContent=d.issuer||'現場ポート';$('#pv-issuer-email').textContent=d.issuerEmail||'';$('#pv-registration').textContent=d.registrationNo?`登録番号 ${d.registrationNo}`:'';let subtotal=0,tax=0;$('#pv-items').innerHTML=d.items.map(x=>{const amount=x.qty*x.unit;subtotal+=amount;tax+=Math.floor(amount*x.tax/100);return `<tr><td>${esc(x.description||'')}</td><td>${x.qty}</td><td>${yen(x.unit)}</td><td>${x.tax}%</td><td>${yen(amount)}</td></tr>`}).join('')||'<tr><td colspan="5">—</td></tr>';$('#pv-subtotal').textContent=yen(subtotal);$('#pv-tax').textContent=yen(tax);$('#pv-total').textContent=$('#pv-grand').textContent=yen(subtotal+tax);$('#pv-due').textContent=d.dueDate||'—';$('#pv-bank').textContent=d.bank||'—';$('#pv-memo').textContent=[d.jobId?`JOB ID: ${d.jobId}`:'',d.memo||''].filter(Boolean).join('\n')||'—'}
...
function resetInvoice(){if(!confirm('請求書を新規状態にしますか？'))return;$('#invoice-form').reset();$('#invoice-form').elements.issueDate.value=today();$('#invoice-form').elements.issuer.value='現場ポート';$('#invoice-form').elements.issuerEmail.value='genbaport@gmail.com';setItems();renderInvoice()}
