(function(){
  ...
  function validApiUrl(url){return /^https:\/\/[a-z0-9-]+\.supabase\.co\/functions\/v1\/akiya-pocket-api$/i.test(url)}
  ...
  async function submit(action,data,timeout){
    if(!validApiUrl(apiUrl))return{ok:false,code:'API_NOT_DEPLOYED',message:'現在、登録・依頼受付を利用できません。時間をおいて再度お試しください。'};
    ...
    try{
      ...
      const response=await fetch(apiUrl,{method:'POST',mode:'cors',credentials:'omit',cache:'no-store',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),signal:controller.signal});
      let result=null;
      try{result=await response.json()}catch{}
      if(!result||typeof result!=='object')throw new Error('サーバーから正しい応答を受信できませんでした。');
      return normalizedResult(result,clientRequestId);
    }catch(err){
      if(err&&err.name==='AbortError')throw new Error('通信がタイムアウトしました。入力内容は保持されています。時間をおいて再度お試しください。');
      throw new Error((err&&err.message)||'通信に失敗しました。時間をおいて再度お試しください。');
